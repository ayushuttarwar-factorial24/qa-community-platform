...
Direct database connection - no FastAPI backend required.
"""
...
# Import direct database services
from services import (
    create_or_update_profile,
    get_profile_by_identifier,
    get_all_profiles_except,
    connect_profiles as db_connect_profiles,
    get_leaderboard as db_get_leaderboard,
    clear_leaderboard_cache,
    get_suggested_matches as db_get_suggested_matches
)
...
# ============================================================================
# DATABASE WRAPPER FUNCTIONS (Direct MongoDB access)
# ============================================================================

def get_profile_by_email(email: str) -> Optional[Dict]:
    """
    Fetch profile by email directly from database.
    ...
    """
    try:
        return get_profile_by_identifier(email)
...
def create_profile(profile_data: Dict[str, Any]) -> Optional[Dict]:
    """
    Creates or updates a profile directly in database.
    ...
    try:
        ...
        if result:
            # Clear leaderboard cache when profile is created/updated
            clear_leaderboard_cache()
        return result
    except Exception as e:
...
def get_suggested_matches(identifier: str, limit: int = None) -> Optional[Dict]:
    """
    Gets suggested profile matches directly from database.
    ...
    Returns:
        Match results dict or None on error
    """
    if limit is None:
    ...
    try:
        # Get source profile
        source_profile = get_profile_by_identifier(identifier)
        if not source_profile:
            st.error("Profile not found")
            return None
        
        # Get all other profiles
        all_profiles = get_all_profiles_except(source_profile.get('user_id'))
        
        # Get matches using direct matching algorithm
        return db_get_suggested_matches(
            source_profile,
            all_profiles,
            limit=limit,
...
def connect_with_profile(initiator_id: str, target_id: str) -> Optional[Dict]:
    """
    Connect with another profile directly via database.
    ...
    """
    try:
        result = db_connect_profiles(initiator_id, target_id)
        
        if result and result.get('success'):
            # Clear leaderboard cache when connection is made
            clear_leaderboard_cache()
            return result
        else:
...
def get_leaderboard() -> Optional[Dict]:
    ...
    Returns:
        Leaderboard dict with profiles
    """
    try:
        return db_get_leaderboard(limit=100)
    except Exception as e:
        ...
        if sent_count > 0:
            with st.expander(f"📤 Connections Sent ({sent_count}/5) - Click to view"):
                ...
                for user_id in connections_sent:
                    try:
                        connected_profile = get_profile_by_identifier(user_id)
